...
def show_login_page():
    st.markdown(
        "<h1 style='text-align: center; color: #1E88E5;'>🏠 House Price Prediction System</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='text-align: center;'>Silakan pilih akses login Anda</p>",
        unsafe_allow_html=True,
    )
    st.write("")
...
def show_user_page():
    # Header
    st.markdown(
        '<div class="main-header">🏠 Prediksi Harga Rumah Jakarta Selatan</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="sub-header">Estimasi harga rumah impian Anda berdasarkan data pasar terbaru.</div>',
        unsafe_allow_html=True,
    )
    ...
    with col_input:
        ...
        with st.form(key="house_form"):
            ...
            submit = st.form_submit_button(
                "🚀 Hitung Estimasi Harga", use_container_width=True
            )
...
def show_admin_page():
    st.markdown(
        '<div class="main-header">📊 Admin Dashboard</div>', unsafe_allow_html=True
    )
    st.markdown(
        '<div class="sub-header">Monitoring Performa Model Regresi.</div>',
        unsafe_allow_html=True,
    )
    ...
    if metrics:
        ...
        if logs_data:
            ...
            with log_col3:
                st.metric(
                    ...
                    delta="✗" if summary.get("error_count", 0) > 0 else None,
                    delta_color="inverse",
                )
            with log_col4:
            ...
            if logs:
                ...
                for log in logs[:20]:  # Show last 20
                    ...
                    if "Linear" in model_used:
                        model_used = "Linear Reg."
                    if "Forest" in model_used:
                    ...
                    log_display.append(
                        {
                            ...
                            "Prediksi (Rp)": f"{log.get('prediction', 0):,.0f}"
                            if log.get("prediction")
                            else "-",
                ...
                st.info(
                    "Belum ada log prediksi. Lakukan prediksi dari halaman User terlebih dahulu."
                )
        else:
            st.warning(
                "⚠️ Tidak dapat mengambil data log. Pastikan API berjalan dengan benar."
            )
        ...
        st.markdown(
            "*Memastikan data yang masuk sesuai dengan pola yang dipelajari sistem*"
        )
                ...
                if features_drift:
                    feature_info = {
                        "LB": {
                            "name": "Luas Bangunan",
                            "unit": "m²",
                            "desc": "Ukuran bangunan rumah",
                        },
                        "LT": {
                            "name": "Luas Tanah",
                            "unit": "m²",
                            "desc": "Ukuran total tanah",
                        },
                        "KT": {
                            "name": "Kamar Tidur",
                            "unit": "kamar",
                    ...
                    st.markdown(
                        "*Grafik perbandingan antara data historis dengan data saat ini*"
                    )
                    ...
                    tab_bar, tab_radar, tab_change, tab_stats = st.tabs(
                        [
                            "Perbandingan Nilai",
                            "Pola Karakteristik",
                            "Tingkat Perubahan",
                    ...
                    with tab_bar:
                        ...
                        fig_bar.update_layout(
                            ...
                            paper_bgcolor="white",
                            font=dict(
                                family="Segoe UI, sans-serif", size=12, color="#495057"
                            ),
                            height=400,
                        ...
                        fig_bar.update_xaxes(
                            showgrid=False, showline=True, linecolor="#dee2e6"
                        )
                        ...
                        st.caption(
                            "Grafik ini menunjukkan perbandingan nilai rata-rata antara data yang dipelajari sistem (historis) dengan data yang masuk saat ini. Semakin mirip kedua batang, semakin konsisten datanya."
                        )

                    with tab_radar:
                        # Normalize data for radar chart (0-100 scale)
                        max_vals = [
                            max(h, c) if max(h, c) > 0 else 1
                            for h, c in zip(chart_historical, chart_current)
                        ]
                        normalized_historical = [
                            (h / m) * 100 for h, m in zip(chart_historical, max_vals)
                        ]
                        normalized_current = [
                            (c / m) * 100 for c, m in zip(chart_current, max_vals)
                        ]
                        ...
                        st.caption(
                            "Grafik radar menunjukkan pola keseluruhan karakteristik rumah. Area biru (saat ini) yang mendekati area abu-abu (historis) menandakan data yang konsisten."
                        )
                    ...
                    with tab_change:
                        ...
                        with col_leg1:
                            st.markdown(
                                '<div style="display:flex;align-items:center;gap:8px;"><span style="width:12px;height:12px;background:#198754;border-radius:50%;display:inline-block;"></span><span style="color:#495057;font-size:0.9em;">Sesuai (perubahan kecil)</span></div>',
                                unsafe_allow_html=True,
                            )
                        with col_leg2:
                            st.markdown(
                                '<div style="display:flex;align-items:center;gap:8px;"><span style="width:12px;height:12px;background:#fd7e14;border-radius:50%;display:inline-block;"></span><span style="color:#495057;font-size:0.9em;">Berubah (perlu perhatian)</span></div>',
                                unsafe_allow_html=True,
                            )
                        with col_leg3:
                            st.markdown(
                                '<div style="display:flex;align-items:center;gap:8px;"><span style="width:12px;height:12px;background:#dc3545;border-radius:50%;display:inline-block;"></span><span style="color:#495057;font-size:0.9em;">Berbeda (perubahan besar)</span></div>',
                                unsafe_allow_html=True,
                            )

                        st.caption(
                            "Grafik ini menunjukkan seberapa besar perubahan nilai dari data historis. Nilai positif (+) berarti lebih tinggi, negatif (-) berarti lebih rendah."
                        )
                    ...
                    with tab_stats:
                        ...
                        st.markdown(
                            "*Analisis menggunakan metode statistik untuk mendeteksi perubahan distribusi data*"
                        )
                        ...
                        for feature, data in features_drift.items():
                            ...
                            cur_mean = data.get(
                                "current_mean", data.get("recent_mean", 0)
                            )
                            ...
                            stats_data.append(
                                {
                                    ...
                                    "Metode Uji": stattest.replace("_", " ").title()
                                    if stattest
                                    else "N/A",
                                    "P-Value": f"{p_val:.4f}"
                        ...
                        st.dataframe(
                            df_stats, use_container_width=True, hide_index=True
                        )
                    ...
                    for idx, (feature, data) in enumerate(items):
                        info = feature_info.get(
                            feature, {"name": feature, "unit": "", "desc": ""}
                        )
                        severity = data.get("severity", "low")
                        ref_val = data.get("reference_mean", 0)
                        cur_val = data.get("current_mean", data.get("recent_mean", 0))
                    ...
                    st.markdown("")
                    st.caption(
                        f"Data dianalisis dari {sample_count} prediksi terakhir • Diperbarui secara otomatis"
                    )
